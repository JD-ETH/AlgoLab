...
typedef delaunay_t::Vertex_iterator viterator_t;
...
bool try_two_color(delaunay_t & trg, K::FT const & rr) {
  for ( viterator_t v= trg.finite_vertices_begin(); v != trg.finite_vertices_end(); ++v){
    v->info().first=0;
  ...
  for ( viterator_t v= trg.finite_vertices_begin(); v != trg.finite_vertices_end(); ++v){
    ...
    if (v->info().second) trg1.insert(v->point()); // 2 colored partial triangles
